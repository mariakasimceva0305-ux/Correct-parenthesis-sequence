    ...
    for (int i = 2*n - 1; i >= 0; i--) {
        if (seq[i] == '(') {
            ...
            if (close > open) {
                ...
                for (int j = i + 1; j < i + 1 + open; j++) {
                    seq[j] = '(';
                }
                for (int j = i + 1 + open; j < (int)(2*n); j++) {
                    seq[j] = ')';
                }
                return 1;
            }
...
void nachalo_iteracii(iterator_t *moj_iterator, unsigned n) {
    ...
    memcpy(moj_iterator->sleduyushaya_posledovatelnost, moj_iterator->tekuyee_znachenie.skobki, 2*n + 1);
...
void pereyti_k_sleduyushemu(iterator_t *moj_iterator) {
    ...
    memcpy(moj_iterator->tekuyee_znachenie.skobki, moj_iterator->sleduyushaya_posledovatelnost, 2*moj_iterator->N_param + 1);
    
    if (!nayti_sleduyushuyu_posledovatelnost(moj_iterator->sleduyushaya_posledovatelnost, moj_iterator->N_param)) {
        free(moj_iterator->sleduyushaya_posledovatelnost);
        moj_iterator->sleduyushaya_posledovatelnost = NULL;
    }
}
